...
# read data
mye_scenic <- open_loom('out_scenic.loom')
...
# RSS analysis
seu_mye = readRDS("~/R/Projects/snRNA_scRNA_hcc/project/Myeloid_cell/Integration_scanvi/myeloid_scanvi.rds")
meta <- seu_mye@meta.data %>% 
  select(patient_id, condition, nCount_RNA, nFeature_RNA, int_clusters)
...
cellinfo <- meta %>% 
  filter(int_clusters %in% c("FOLR2+ TAM", "IFN-TAM", "Inflam-TAM", "Kupffer cell", "TREM2+ LAM"))

group <- cellinfo %>% 
  select(int_clusters) %>% 
  mutate(condition = case_when(
    int_clusters == 'TREM2+ LAM' ~ 'TREM2p', 
    TRUE ~ 'TREM2n'
  )) %>%
